 ...
 * provided Dijkstra class with missing methods implemented
 *
 */
...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
	  double dist = Math.pow((ux - vx), 2) + Math.pow((uy - vy), 2);
	  dist = Math.sqrt(dist);
      return dist; 
  }
  ...
  public void doDijkstra(String s) {
	  //README = implemented Comparable on Vertex
	  Vertex[] vArr = new Vertex[vertexNames.size()];
	  int i = 0;
      for (Vertex v : vertexNames.values()){
      ...
      //build binary min heap of vertices for O(log N) access of next vertex to be marked as known
      BinaryHeap<Vertex> heap = new BinaryHeap<Vertex>(vArr);
      int vNum = 0;
      while (vNum < vertexNames.size()){
    	  Vertex v = heap.deleteMin();
    	  v.known = true;
    	  for (Edge w : v.adjacentEdges){
    		  if (!w.target.known){
    			  double dist = w.distance;
    			  //update stored distance if the new distance is less
    			  if (dist + v.distance < w.target.distance){
    				  w.target.distance = dist + v.distance;
    				  heap.decreaseKey(w.target);
    				  w.target.prev = v;
    			  }
    		  }
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    ...
    while (!end.equals(start)){
    	Vertex prev = end.prev;
    	//find the corresponding edge object to add it to the List
